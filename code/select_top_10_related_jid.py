...
def create_job_description_cosine_similarity_matrix(job_description_features_df):
    ...
    for i in range(0, len(cosine_similarity_df)):
        ...
        array_df = job_description_features_df.loc[:, job_description_features_df.columns != 'jid']
        array_A = array_df.to_numpy()
        array_B = array_df.to_numpy()[i]
        cosine_df_one_jid[jid] = calculate_cosine_similarity(array_A, array_B)
        cosine_similarity_df = pd.concat([cosine_similarity_df, cosine_df_one_jid], axis=1, join='inner')
...
def jid_top_10_related_job(job_description_cosine_similarity_df):
    ...
    for jid in job_description_cosine_similarity_df.index:
        top_25_jid_list = job_description_cosine_similarity_df[jid].sort_values(ascending = False).index[1:11]
...
if __name__ == '__main__':
    ...
    job_description_features_df = extract_features(clean_job_des_df, 'clean_description_n_v_j_only')

    job_description_cosine_similarity_df = create_job_description_cosine_similarity_matrix(job_description_features_df)

    top_10_jid_df = jid_top_10_related_job(job_description_cosine_similarity_df)
    top_10_jid_df = top_10_jid_df.rename(columns = {0:'top_1', 1:'top_2', 2:'top_3', 3:'top_4', 4:'top_5', 5:'top_6', 6:'top_7', 7:'top_8', 8:'top_9', 9:'top_10'})
