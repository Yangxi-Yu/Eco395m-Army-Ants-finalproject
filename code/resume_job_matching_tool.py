...
def add_date_diff_column(job_posted_date_df):
    ...
    for i in range(0, len(job_posted_date_df)):
        if pd.isnull(job_posted_date_df.loc[i, "date"]):
            ...
            job_posted_date_df.loc[i, "date_diff"] = abs(
                job_posted_date_df["date"][i].date() - date.today()
            ).days
...
def data_source_filter(position, degree, location, exp_level, date_post):
    ...
    job_description_df_part = job_description_df[
        ["jid", "title", "clean_description_n_v_j_only", "degree", "exp_level"]
    ]
    job_basic_info_df_part = job_basic_info_df[["jid", "location"]]
    job_posted_date_df_part = job_posted_date_df[["jid", "date"]]

    data_source_df = job_description_df_part.merge(
        job_basic_info_df_part, how="left", on="jid"
    )
    ...
    data_source_filter_df = (
        data_source_df.loc[
            data_source_df["title"]
            == (position_da or position_de or position_ds or positon_general)
        ]
        .loc[
            data_source_df["degree"] == (degree_phd or degree_master or degree_bachelor)
        ]
        .loc[data_source_df["exp_level"] == exp_level]
        .loc[
            data_source_df["location"]
            == (location_ny or location_ca or location_tx or location)
        ]
...
def input_txt_return_top_50_related_job(job_description_features_df, cleaned_input_txt):
    ...
    for jid in job_description_features_df.index:
        ...
        array_B = convert_cleaned_input_to_array(
            cleaned_input_txt, job_description_features_df
        )
        jid_cosine_similarity_dict[jid] = calculate_cosine_similarity(array_A, array_B)

    sorted_jid_cosine_similarity_dict = dict(
        sorted(
            jid_cosine_similarity_dict.items(), key=lambda item: item[1], reverse=True
        )
    )
    jid_cosine_similarity_df = pd.DataFrame(
        sorted_jid_cosine_similarity_dict.items(), columns=["jid", "cosine_similarity"]
    )
